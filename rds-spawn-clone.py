...
def main(argv):
  ...
  cmd = "aws rds restore-db-instance-from-db-snapshot --db-instance-identifier %s --tags Key=rds-dr-walkaway,Value=%s --db-snapshot-identifier %s --db-instance-class %s" % (
    restoreName,
    snapshot,
    snapshot,
    instanceType
